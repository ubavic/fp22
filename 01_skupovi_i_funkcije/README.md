...
Dakle, za nas *skup* predstavlja nekakvu kolekciju elemenata. Činjenicu da element `x` pripada skupu `S` obeležavamo sa `x ∈ S`.
...
Zanimljivo je primetiti sledeće, ako je `f` surjektivna funkcija, tada iz `g₁ ∘ f = g₂ ∘ f` sledi `g₁ = g₂`. Slično, ako je `f` injekcija tada iz `f ∘ g₁ = f ∘ g₂` sledi da je `g₁ = g₂`.
